...
public class StoriesFacade {
    ...
    public Story buildStory(FileInputStream fileInputStream, Path path) throws IOException, ConnectorException {
        LOG.info("Adding story: " + path.getFileName().toString()); // TODO: should create story with storyName, not path ?
        return storiesHolder.add(new Story(grammar, fileInputStream, path));
    }
    ...
    public void refreshFromDisk() {
        ...
        for (String storyName : storyNames) {
            try {
                if (getStoryByName(storyName) == null) {
                    try (FileInputStream fileInputStream = DiskHelper.getStoryAsStream(storyName)) {
                        buildStory(fileInputStream, new File(storyName).toPath());
                    }
