...
.controller('StoryCtrl', ['$http', '$scope', '$routeParams', '$sce', function($http, $scope, $routeParams, $sce) {
    ...
    if ($scope.originalStoryName=='new') {
        ...
        $http.get('../ws/admin/story/'+$scope.originalStoryName)
    ...
    $scope.validate = function() {
        $http.post('../ws/admin/validation/story', $scope.story.storyText)
            .then(
    ...
    $scope.submit = function() {
        ...
        $http.post('../ws/admin/story/'+$scope.originalStoryName, data)
            .then(
                function(data, status) {
                    if (data.data=="Ok") {
